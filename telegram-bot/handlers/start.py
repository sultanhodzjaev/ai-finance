...
async def cmd_start(message: Message):
    ...
    if ref_param.startswith("ref_"):
        ...
        if applied:
            ...
            # Юзер уже зарегистрирован — реф не активируется. Объясняем явно,
            # иначе тот, кто прислал ссылку, не понимает почему бонус не пришёл.
            referral_note = (
                "\nℹ️ Реферальная ссылка не активирована — она работает только при "
                f"первой регистрации. Если хочешь сам приглашать друзей и получать +{REFERRAL_BONUS_DAYS} "
                "дней Premium за каждого — /invite.\n"
            )
